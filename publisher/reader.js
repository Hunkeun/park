...
(function () {
    ...
    function ttsPreprocess(text) {
        ...
        out = out.replace(/[▶▷◀◁▲△▼▽★☆◆◇●○◎■□♥♡♦♣♠※☞☜→←↑↓⇒⇐⇑⇓]+/g, ' ');
        // 1.7) 한자어의 한자 표기 괄호 제거. 예: "수원(水源)" → "수원".
        //      한국어로 이미 읽은 단어를 한자로 다시 발화하는 중복을 막는다.
        //      괄호 안이 "순수 한자만"일 때만 제거 (반각·전각 괄호 모두). 한글·숫자·영문이
        //      섞여 있으면 의미 정보일 가능성이 높으니 그대로 둔다 (예: "(2026년)", "(漢字, hànzì)").
        out = out.replace(/\s*[\(（]\s*[一-鿿]+\s*[\)）]/g, '');
        // 2) 성경 인용 한자어 한글 변환
